...
test("list generation supports conditions, save dry-run, CSV upload preview, and saved list reuse", async ({ page }, testInfo) => {
  ...
  await expect(page.getByText("Saved list comparison")).toBeVisible();
  await expect(page.getByText("Choose another saved list")).toBeVisible();
  await page.getByLabel("Compare with another saved list").selectOption("bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb");
  ...
  await expect(page.locator("main")).toContainText("Base");
  await expect(page.locator("main")).toContainText("Target");
  await expect(page.locator("main")).toContainText("Removed");
  ...
  await expect(appAlert(page)).toContainText("Select a different saved list to compare.");
